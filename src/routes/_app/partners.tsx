...
import { CheckCircle2, ExternalLink, Loader2, MapPin, Pencil, Plus, Trash2, XCircle } from "lucide-react";
        ...
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          {editing && (
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Nome completo *"><Input autoComplete="name" value={editing.full_name ?? ""} onChange={(e) => set("full_name", e.target.value)} /></Field>
              <Field label="CPF ou CNPJ"><Input value={editing.cpf_cnpj ?? ""} onChange={(e) => set("cpf_cnpj", maskCpfCnpj(e.target.value))} /></Field>
              <Field label="Telefone / WhatsApp"><Input autoComplete="tel" value={editing.phone ?? ""} onChange={(e) => set("phone", maskPhone(e.target.value))} /></Field>
              <Field label="E-mail"><Input type="email" autoComplete="email" value={editing.email ?? ""} onChange={(e) => set("email", e.target.value)} /></Field>
              <Field label="RG"><Input value={editing.rg ?? ""} onChange={(e) => set("rg", maskRg(e.target.value))} /></Field>
              <Field label="CNH"><Input inputMode="numeric" value={editing.cnh ?? ""} onChange={(e) => set("cnh", maskCnh(e.target.value))} /></Field>
              <div className="sm:col-span-2"><AddressFields form={editing} set={set} searchingCep={searchingCep} lookupCep={lookupCep} /></div>
              <div className="sm:col-span-2 rounded-md border bg-muted/10 p-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field label="Nome do proprietario/responsavel"><Input value={editing.property_owner_name ?? ""} onChange={(e) => set("property_owner_name", e.target.value)} /></Field>
                  <Field label="Telefone / WhatsApp do proprietario"><Input value={editing.property_owner_phone ?? ""} onChange={(e) => set("property_owner_phone", maskPhone(e.target.value))} /></Field>
                  <div className="sm:col-span-2"><Field label="E-mail do proprietario"><Input type="email" value={editing.property_owner_email ?? ""} onChange={(e) => set("property_owner_email", e.target.value)} /></Field></div>
                  <div className="sm:col-span-2"><Field label="Endereco do imovel indicado"><Input value={editing.property_address ?? composeIndicatedPropertyAddress(editing)} onChange={(e) => set("property_address", e.target.value)} /></Field></div>
                  <div className="sm:col-span-2"><Field label="Dados adicionais do imovel"><Textarea rows={3} value={editing.property_notes ?? ""} onChange={(e) => set("property_notes", e.target.value)} /></Field></div>
                </div>
              </div>
              <div className="sm:col-span-2 rounded-md border border-primary/20 bg-primary/5 p-3">
                ...
                <p className="mb-3 text-xs text-muted-foreground">Preencha somente em ambiente interno, apos aprovacao do parceiro.</p>
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field label="Forma de recebimento">
                    <Select value={editing.payment_preference ?? "none"} onValueChange={(value) => set("payment_preference", value === "none" ? null : value)}>
                  ...
                  <Field label="Chave Pix (se usar Pix)"><Input value={editing.pix_key ?? ""} onChange={(e) => set("pix_key", e.target.value)} /></Field>
                  <div className="sm:col-span-2"><Field label="Dados bancarios ou instrucoes de pagamento"><Textarea rows={3} value={editing.payment_details ?? ""} onChange={(e) => set("payment_details", e.target.value)} /></Field></div>
                </div>
              </div>
              <div className="sm:col-span-2"><Field label="Observacoes"><Textarea rows={3} value={editing.notes ?? ""} onChange={(e) => set("notes", e.target.value)} /></Field></div>
  ...
  return (
    <section className="rounded-md border bg-muted/10 p-3">
      ...
      <div className="grid gap-3 md:grid-cols-[180px_1fr]">
        ...
        <Field label="Rua / Logradouro"><Input autoComplete="address-line1" value={form.street ?? ""} onChange={(e) => set("street", e.target.value)} /></Field>
        <Field label="Numero"><Input autoComplete="address-line2" value={form.number ?? ""} onChange={(e) => set("number", e.target.value)} /></Field>
      ...
      <div className="mt-3"><Field label="Endereco completo"><Input value={form.address ?? composeAddress(form)} onChange={(e) => set("address", e.target.value)} /></Field></div>
    </section>
  );
}
