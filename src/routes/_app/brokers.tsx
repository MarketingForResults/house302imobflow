...
function BrokersPage() {
  ...
  function openEdit(broker: any) {
    setForm({
      ...broker,
      registration_status: broker.registration_status ?? (broker.creci ? "regular" : "irregular"),
    });
    setOpen(true);
  }
  ...
  async function save() {
    ...
    if (form.registration_status === "regular" && !form.creci?.trim())
      return toast.error("Informe o numero do registro CRECI");
  ...
  return (
    <div>
      <PageHeader
        title="Corretores"
        description="Parceiros e captadores"
        actions={
          <Dialog
            open={open}
            onOpenChange={(value) => {
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      value={form.registration_status ?? "regular"}
                      onValueChange={(value) =>
                        setForm({
                          ...form,
                          registration_status: value,
                          creci: value === "irregular" ? null : form.creci,
                        })
                      }
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="irregular">
                          Corretor sem registro profissional (Autonomo)
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                ...
                {form.registration_status !== "irregular" && (
                  <div>
                    <Label>Numero do registro CRECI</Label>
                    <Input
                      value={form.creci ?? ""}
              ...
              {brokers.map((broker: any) => (
                <tr key={broker.id} className="border-t">
                  ...
                  <td className="px-4 py-3">
                    {broker.registration_status === "irregular"
                      ? "Autonomo sem registro"
                      : (broker.creci ?? "-")}
                  </td>
                  <td className="px-4 py-3">{broker.phone ?? "-"}</td>
                  <td className="px-4 py-3">
                    {broker.commission_pct ? `${broker.commission_pct}%` : "-"}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Switch
                        checked={broker.active}
                        onCheckedChange={(value) => toggleActive(broker.id, value)}
                      />
                      <Badge variant={broker.active ? "default" : "outline"}>
