...
function PropertyEdit() {
  ...
  const [form, setForm] = useState<any>({
    ...
    status: "available",
    country: "Brasil",
    workflow_status: "capture_pending",
    listing_purpose: "sale_rent",
    planned_furniture: false,
    furnished: false,
    financed: false,
    accepts_trade: false,
    exclusive: false,
  ...
  async function advanceWorkflow(workflow_status: string, message: string) {
    ...
    if (
      isAdmin &&
      ["inspection_pending", "ready_to_publish", "rejected"].includes(workflow_status)
    ) {
      const {
        data: { user },
      } = await supabase.auth.getUser();
  ...
  async function uploadImages(files: FileList | File[]) {
    ...
    if (list.length > remaining)
      toast.warning(`Apenas ${remaining} imagem(ns) serão enviadas (limite 30)`);
      ...
      <div className="grid gap-6 p-4 md:p-8 lg:grid-cols-3">
        <Section title="Etapa da captação" className="lg:col-span-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <Badge variant={form.workflow_status === "rejected" ? "destructive" : "outline"}>
                {WORKFLOW_LABEL[form.workflow_status] ?? form.workflow_status}
              </Badge>
              <p className="mt-2 text-xs text-muted-foreground">
                O imóvel só estará apto para divulgação após a aprovação administrativa da vistoria
                técnica.
              </p>
            </div>
            {!isNew && (
              <div className="flex flex-wrap gap-2">
                {form.workflow_status === "capture_pending" && (
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() =>
                      advanceWorkflow("registration_in_progress", "Atendimento iniciado")
                    }
                  >
                    Iniciar atendimento
                  </Button>
                )}
                {form.workflow_status === "registration_in_progress" && (
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() =>
                      advanceWorkflow("awaiting_admin_review", "Cadastro enviado para aprovação")
                    }
                  >
                    Enviar cadastro para aprovação
                  </Button>
                )}
                {isAdmin && form.workflow_status === "awaiting_admin_review" && (
                  <Button
                    type="button"
                    onClick={() =>
                      advanceWorkflow(
                        "inspection_pending",
                        "Cadastro aprovado e enviado para vistoria",
                      )
                    }
                  >
                    Aprovar e enviar para vistoria
                  </Button>
                )}
                {isAdmin &&
                  ["awaiting_admin_review", "awaiting_inspection_review"].includes(
                    form.workflow_status,
                  ) && (
                    <Button
                      type="button"
                      variant="destructive"
                      onClick={() => advanceWorkflow("rejected", "Imóvel reprovado")}
                    >
                      Reprovar
                    </Button>
                  )}
        ...
        <Section title="Origem da captação e proprietário" className="lg:col-span-3">
          <div className="grid gap-3 md:grid-cols-3">
            <Field label="Finalidade">
              <Select
                value={form.listing_purpose ?? "sale_rent"}
                onValueChange={(value) => set("listing_purpose", value)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <Field label="Parceiro indicador">
              <Select
                value={form.capture_partner_id ?? "none"}
                onValueChange={(value) => set("capture_partner_id", value)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Captação direta" />
                </SelectTrigger>
                <SelectContent>
            ...
            <Field label="Cliente proprietário já cadastrado">
              <Select value={form.client_id ?? "none"} onValueChange={selectOwner}>
                <SelectTrigger>
                  <SelectValue placeholder="Selecione se existir" />
                </SelectTrigger>
                <SelectContent>
            ...
            <Field label="Nome do proprietário ou responsável">
              <Input
            ...
            <Field label="CPF do proprietário">
              <Input
                ...
                onChange={(e) => set("owner_cpf", maskCpf(e.target.value))}
              />
            </Field>
            ...
            <div className="md:col-span-3">
              <Field label="Observações da captação">
                <Textarea
                  rows={3}
                  value={form.capture_notes ?? ""}
                  onChange={(e) => set("capture_notes", e.target.value)}
                />
              </Field>
            </div>
            {isAdmin && (
              <div className="md:col-span-3">
                <Field label="Parecer administrativo">
                  <Textarea
                    rows={3}
        ...
        <Section title="Comercial">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Venda mínima (R$)">
              <Input
                type="number"
                value={form.sale_min_price ?? ""}
                onChange={(e) => set("sale_min_price", e.target.value)}
              />
            </Field>
            <Field label="Venda máxima (R$)">
              <Input
                type="number"
                value={form.sale_max_price ?? ""}
                onChange={(e) => set("sale_max_price", e.target.value)}
              />
            </Field>
            <Field label="Aluguel mínimo (R$)">
              <Input
                type="number"
                value={form.rental_min_price ?? ""}
                onChange={(e) => set("rental_min_price", e.target.value)}
              />
            </Field>
            <Field label="Aluguel máximo (R$)">
              <Input
                type="number"
                value={form.rental_max_price ?? ""}
                onChange={(e) => set("rental_max_price", e.target.value)}
              />
            </Field>
          </div>
        ...
        <Section title="Características" className="lg:col-span-2">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
            ...
            <Toggle
              ...
              v={form.planned_furniture}
              onChange={(v) => set("planned_furniture", v)}
            />
            <Toggle label="Financiável" v={form.financed} onChange={(v) => set("financed", v)} />
            <Toggle
              label="Aceita permuta"
              v={form.accepts_trade}
              onChange={(v) => set("accepts_trade", v)}
            />
            <Toggle
              label="Exclusividade"
              v={form.exclusive}
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            ...
            <div className="md:col-span-4 flex flex-wrap gap-2 items-center justify-between border-t border-b py-4 my-2">
              <span className="text-xs text-muted-foreground">
                Preencha o endereço acima e clique ao lado para buscar as coordenadas geográficas
                automaticamente
              </span>
              <Button
                type="button"
                ...
                size="sm"
                onClick={() =>
                  updateCoordinates(form.address, form.neighborhood, form.city, form.state)
                }
                disabled={fetchingCoords || !form.address || !form.city || !form.state}
              >
        ...
        {!isNew && (
          <Section
            title={`Galeria de imagens (${existing?.property_images?.length ?? 0}/30)`}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
              {existing?.property_images?.map((img: any) => (
                ...
                >
                  ...
                  {img.is_cover && (
                    <div className="absolute left-1 top-1 rounded bg-accent px-1.5 py-0.5 text-[10px] font-medium text-accent-foreground">
                      CAPA
                    </div>
                  )}
                  <div className="absolute inset-0 hidden items-center justify-center gap-1 bg-black/60 group-hover:flex">
                    ...
                    <button
                      ...
                      className="rounded bg-white/10 p-1.5 text-white hover:bg-destructive"
                      title="Excluir"
                    >
        ...
        {!isNew && (
          <Section title="Documentos do imóvel" className="lg:col-span-3">
            <EntityDocuments
              entityType="property"
              entityId={id}
              title="Matrícula, certidões e outros documentos"
            />
          </Section>
        )}
      </div>
