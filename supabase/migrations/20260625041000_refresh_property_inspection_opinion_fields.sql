-- Ensure inspection opinion fields exist and refresh PostgREST schema cache.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    FROM pg_constraint
    WHERE conrelid = 'public.property_inspections'::regclass
      AND conname = 'property_inspections_reminder_minutes_check'
  ) THEN
    ALTER TABLE public.property_inspections
      ADD CONSTRAINT property_inspections_reminder_minutes_check
      CHECK (reminder_minutes IN (0, 15, 30, 60, 120, 1440)) NOT VALID;
  END IF;
